...
## -------------------------------------------------------------------
## Fixed experiment settings (NONADDITIVE)
## -------------------------------------------------------------------
N  <- 200
M  <- 50
N1 <- 50
...
# only non-oracle selection here
oracle <- 0L

# true signal set for F1 (first 10 features)
signal_set <- 1:10

# experiment loops (from your nonadditive run_adamp.R)
snr_list <- c(4, 6, 8, 10)
num_rep  <- 30
...
# selection rule: same as your run_adamp.R (use (num_epochs-1) if possible)
extract_selected_features_nonoracle <- function(res, delta) {
...
for (snr in snr_list) {
  ...
  # accumulate rep-wise metrics: rows=grid points, cols=reps
  ...
  for (rr in seq_along(rep_list)) {
    ...
    # placeholder (kept consistent with your scripts)
    ...
    for (gg in seq_len(nrow(grid))) {
      ...
      # control AdaMP randomness per rep
      set.seed(rep)
      ...
      selected_idx <- extract_selected_features_nonoracle(res, delta)
  ...
  # per-SNR grid summary CSV
  grid_summary <- data.frame(
    n_ratio = grid$n_ratio,
    m_ratio = grid$m_ratio,
    avg_loo_final = apply(loo_mat, 1, mean, na.rm = TRUE),
    avg_f1        = apply(f1_mat,  1, mean, na.rm = TRUE),
    stringsAsFactors = FALSE
  )
