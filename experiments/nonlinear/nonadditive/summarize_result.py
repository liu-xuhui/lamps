...
for permute in [0,1]:
    for oracle in [0,1]:
        for corr in corr_list:
            ...
            file_path = os.path.join("temp_results/temp_compare_result", f"adamars_nonlinear_nonadd_oracle{oracle}_permute{permute}_result_dict.pkl")
            if os.path.exists(file_path):
                with open(f"temp_results/temp_compare_result/adamars_nonlinear_nonadd_oracle{oracle}_permute{permute}_result_dict.pkl", "rb") as f:
                    temp_result_dict = pickle.load(f)
                    com_method_result_dict["adammars"] = temp_result_dict[corr]["adammars"]

            file_path = os.path.join("temp_results/temp_compare_result", f"adaspam_nonlinear_nonadd_oracle{oracle}_permute{permute}_result_dict.pkl")
            if os.path.exists(file_path):
                with open(f"temp_results/temp_compare_result/adaspam_nonlinear_nonadd_oracle{oracle}_permute{permute}_result_dict.pkl", "rb") as f:
                    temp_result_dict = pickle.load(f)
