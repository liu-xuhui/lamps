    ...
    for snr in snr_list:
        for rep in range(num_simus):
            for corr in corr_list:
                ...
                out_df = pd.DataFrame()
                out_df["var"] = selected
                out_df.to_csv(f"temp_results/nonlinear/nonadditive/knockoff03_selected_corr{corr}_snr{snr}_permute{permute}_oracle{oracle}_rep{rep}.csv", index=False)

                selected = Konckoff_select(X, Y, 0.2)
                out_df = pd.DataFrame()
                out_df["var"] = selected
                out_df.to_csv(f"temp_results/nonlinear/nonadditive/knockoff02_selected_corr{corr}_snr{snr}_permute{permute}_oracle{oracle}_rep{rep}.csv", index=False)

                selected = Konckoff_select(X, Y, 0.1)
                out_df = pd.DataFrame()
                out_df["var"] = selected
                out_df.to_csv(f"temp_results/nonlinear/nonadditive/knockoff01_selected_corr{corr}_snr{snr}_permute{permute}_oracle{oracle}_rep{rep}.csv", index=False)
