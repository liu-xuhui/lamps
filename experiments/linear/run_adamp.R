...
snr_list    <- c(1, 2, 5, 10, 20)
num_rep     <- 10
p           <- 500
corr_list   <- c(0, 0.5, 0.9)
oracle_list <- c(0, 1)

out_dir <- file.path(root, "temp_results", "linear")
  ...
  for (snr in snr_list) {
    ...
    for (rep in 0:(num_rep - 1)) {
      ...
      data_path <- file.path(
        ...
        sprintf("linear_corr%g_snr%g_permute%d_rep%d.csv",
      ...
      for (oracle in oracle_list) {
        ...
        if (oracle == 0) {
          linear_select1 <- which(prob_F_use >= delta * 0.5)
        } else {
          linear_select1 <- order(prob_F_use, decreasing = TRUE)[1:number_signals]
        }

        out_path <- file.path(
            out_dir,
            sprintf(
              "adamlinear_selected_corr%g_snr%g_permute%d_oracle%d_rep%d.csv",
              corr, snr, permute, oracle, rep
            )
        )
        

        write.csv(data.frame(var = linear_select1),
